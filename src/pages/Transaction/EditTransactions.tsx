...
import CustomizedInput from "../../components/Input/CustomizedInput";

const EditTransactions = () => {
    const { id } = useParams<{ id: string }>();
    const transaction = transactionData.find(td => td.id === Number(id));
    if (!transaction) {
    ...
    return (
        <>
            <div className="text-3xl text-black font-medium flex items-center mb-6">
                <Link to={'/transaction/transactions'} className="inline-block mx-2 p-2 rounded-xl border duration-500 border-transparent hover:border-black">
            ...
            <div className="bg-white py-8 px-12 rounded-xl">
                ...
                <div className="flex justify-end mt-10 gap-4">
                    <Link to={'/transaction/transactions'} className="min-w-30 text-center py-2 bg-primary rounded-lg text-white">
                        Save
                    </Link>
                    <Link to={'/transaction/transactions'} className="min-w-30 text-center py-2 bg-gray rounded-lg text-black">
                        Cancel
                    </Link>
                </div>
            </div>
